...
#define cf_safe_strcpy(dst, src, n, max)                                                           \
	cf_safe_strcpy_internal(dst, src, n, max, __FILE__, __LINE__)
static int cf_safe_strcpy_internal(char *dst, const char *src, int n, int max, const char *file,
								   int line)
{
	int c;
...
int cf_read_file(cf_dir_t *dir, cf_file_t *file)
{
	...
	file->size =
		((size_t)dir->fdata.nFileSizeHigh * (max_dword + 1)) + (size_t)dir->fdata.nFileSizeLow;
	cf_get_ext(file);
